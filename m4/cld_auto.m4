...
dnl very similar to CLD_AUTO_RESULT, but does not finalize the
dnl detection; it assumes that more checks will follow.
AC_DEFUN([CLD_AUTO_PRE], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$var` != xno" && eval "test x`echo '$'$found` = xno"; then
                CLD_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([CLD_AUTO_RESULT], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$found` = xyes"; then
                CLD_AUTO_ENABLED([$name], [$feature])
	else
                CLD_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([CLD_AUTO_PKG], [
